...
GET_WORKING_CH = 'AA0000AA'
SET_WORKING_CH = 'AD0001FFAD'
...
class UHF():
    ...
    def getWorking_CH(self):
        data = self.send_command([STARTBYTE, GET_WORKING_CH, ENDBYTE])
        time.sleep(0.5)
        rec_data = self.serial.read(8)
        if rec_data is not None:     
            return ['{:02x}'.format(x) for x in rec_data]
        
    def setWorking_CH(self):
        data = self.send_command([STARTBYTE, SET_WORKING_CH, ENDBYTE])
        time.sleep(0.5)
        rec_data = self.serial.read(8)
        if rec_data is not None:     
            return ['{:02x}'.format(x) for x in rec_data]
