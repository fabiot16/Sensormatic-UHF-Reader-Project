...
void WiFiManager::setupHTTPServer(){
  ...
  server->on(WM_G(R_rfid),       std::bind(&WiFiManager::handleRFID, this));
  server->on(WM_G(R_rfidsave),   std::bind(&WiFiManager::handleRFIDSave, this));
...
/**
 * HTTPD CALLBACK RFID param page handler
 */
void WiFiManager::handleRFID(){
  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_VERBOSE,F("<- HTTP RFID"));
  #endif
  String page = getHTTPHead(S_titlerfid); // @token options @todo replace options with titlerfid
  String str  = FPSTR(HTTP_RFID_MAIN); // @todo custom title
  str.replace(FPSTR(T_t),S_titlerfid);
  page += str;

  page += F("<form method='POST' action='/rfidsave'>");

  // Placeholder values read from the file
  String rfidValues[4];
  File rfidFile = SPIFFS.open("/rfid.txt", "r");
  if (rfidFile) {
    for (int i = 0; i < 4 && rfidFile.available(); i++) {
      rfidValues[i] = rfidFile.readStringUntil('\n');
      Serial.println(rfidValues[i]);
    }
    rfidFile.close();
    Serial.println("End read");
  }

  page += "<label for='endpoint'>Endpoint</label> <input id='endpoint' name='endpoint' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='" + rfidValues[0] + "'><br/><br/>";
  page += "<label for='power'>Power dBm</label> <input id='power' name='power' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='" + rfidValues[1] + "'><br/><br/>";
  page += "<label for='send'>Send Timer</label> <input id='send' name='send' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='" + rfidValues[2] + "'><br/><br/>";
  page += "<label for='clear'>Clear Timer</label> <input id='clear' name='clear' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='" + rfidValues[3] + "'>";
  page += FPSTR(HTTP_FORM_END);
...
/** 
 * HTTPD CALLBACK save form and redirect to RFID config page again
 */
void WiFiManager::handleRFIDSave() {
  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_VERBOSE,F("<- HTTP RFID save "));
  DEBUG_WM(DEBUG_DEV,F("Method:"),server->method() == HTTP_GET  ? (String)FPSTR(S_GET) : (String)FPSTR(S_POST));
  #endif
  handleRequest();
  ...
  Serial.println(send);
  String clear = server->arg("clear");
  Serial.println(clear);

  // Save the RFID values to the "rfid.txt" file
  File rfidFile = SPIFFS.open("/rfid.txt", "w");
  if (rfidFile) {
    rfidFile.println(endpoint);
    rfidFile.println(power);
    rfidFile.println(send);
    rfidFile.println(clear);
    rfidFile.close();
    Serial.println("End write");
  }
  // Redirect back to the RFID config page
  server->sendHeader("Location", "/rfid", true);
  server->send(302, "text/plain", "");

  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_DEV, F("Saved RFID values"));
  #endif
}
